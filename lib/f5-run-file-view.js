...
export default class F5RunFileView {
  ...
  execute() {
    ...
    this.readDir(this.projectFilePath, (path) => this.projectrun(path), (path) => this.shellprojectrun(path), (path) => this.singlefilerun(path), currentFilePath);
  }
  ...
  readDir(dir, succ, partial, fail, filepath){
    fs = require('fs');
