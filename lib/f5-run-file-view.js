...
export default class F5RunFileView {
  ...
  execute() {
    ...
    if ((typeof projectFilePath) !== "string"){
      console.log("ERRRRRRROR: illegal type of projectfilepath: "+(typeof projectFilePath)+(projectFilePath instanceof String));
      return;
    }
    fs.readdir(projectFilePath, function(err, files) {
      if (err){
        console.log(err);
        jsonPath = null;
        return;
      ...
      files.forEach(file => {
        if (file === "launch.json"){
          jsonPath = projectFilePath + "/"+file;
          console.log(jsonPath);
          return;
        }
      });
      //call the default commands here
    });
    console.log("json path from main: "+jsonPath);
  }
}
